...
public class HomePageNonUSMobileAndroid extends MobileBasePage {
	Boolean flag;
	...
	private By fourweathercardsMobile = By
			.cssSelector("div > div.sliding-panel.three-day-panel.three-day-forecast.full-mobile-width");
	...
	private By byLogo2 = By.cssSelector(
	...
	public Boolean verifyTemperatureForAllLocationsOfCountry() {
		...
		actualLocationsInTotal = locations.size();
		for (int i = 1; i <= locations.size(); i++) {
			if (getDriver().findElements(By.cssSelector("div > a:nth-child(" + i + ") > span.text.temp")).size() > 0) {
				flag = true;
				continue;
			} else
				flag = false;
	...
	public Boolean clickBreadCrumbMenulanguageSelectMobile() {
		getDriver().executeJavaScript("seetest:client.click(\"NATIVE\", \"text=Español\", 0, 1)");
		Sleeper.sleep(3);
		return true;
	...
	public Boolean clickBreadCrumbMenulanguageSelectMobileEnglish() {
		getDriver().executeJavaScript(
				"seetest:client.swipeWhileNotFound(\"up\", 1000, 1000, \"NATIVE\", \"xpath=//*[@text='English (UK)']\", 0, 100, 5, true))");
		Sleeper.sleep(3);
		getDriver().executeJavaScript("seetest:client.click(\"NATIVE\", \"text=English (US)\", 0, 1)");
		Sleeper.sleep(3);
		return true;
	}
