...
public class MinutecastMobile extends MobileBasePage
{
	...
	private By byminutecastHighlighted =By.cssSelector("body > div > div.page-subnav > div > div > div.subnav-items > a.subnav-item.active > h1");
	...
	private By byradarTooltipactive=By.cssSelector("div.map-container.full-mobile-width.content-module > div.map-header > div");
   ...
   private By byradartooltipTitle = By.cssSelector("div.tooltip-overlay > div.tooltip-header > span");
	...
	private By byPastCTA=By.cssSelector("div.map-container.full-mobile-width.content-module > div.map-tools > div.radar-controls.toggle-container > div.map-button.is-active");
...
//		wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[@class='search-results']//*")));
	...
	public String minutecastTabHighlighted() {
		WebPageLoaded.isDomInteractive();
		Sleeper.sleep(4);
		WebElement ele = getDriver().findElement(byminutecastHighlighted);
		String result = (String) DriverManager.getWebDriver().executeJavaScript(
				"return getComputedStyle(document.querySelector(\"body > div > div.page-subnav > div > div > div.subnav-items > a.subnav-item.active > h1\")).border-top;",  
				ele);
		System.out.println("TAB COLOR DISLAYED IN RGB" +result);
		
		return Color.fromString(result).asHex();
	}
	
	//body > div > div.page-subnav > div > div.subnav-items > a.subnav-item.active
