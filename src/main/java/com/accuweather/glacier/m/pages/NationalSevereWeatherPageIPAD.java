...
public class NationalSevereWeatherPageIPAD extends MobileBasePage {

	private By byTextAtBottomOfMap = By
			.cssSelector("div.map-container.full-mobile-width.severe-map-container > div.map-tools > div");
	private By byZoomInForMoreDetailsTextBottomOfMap = By
			.cssSelector("div.content-module > div.map-severe.content-module > div.severe-legend > div");
	...
	private By byTitleOfMap = By
			.cssSelector("div.map-container.full-mobile-width.severe-map-container > div.map-header > h2");
	...
	public String verifyToolTipClosed() {
		...
		String result = (String) DriverManager.getWebDriver().executeJavaScript(
				"return getComputedStyle(document.querySelector(\"body > div > div:nth-child(5) > div > div.page-column-1 > div:nth-child(1) > div.map-severe.content-module > div > div.map-header > div > div.icon-tooltip\")).color;",
				ele);
		System.out.println("result > > > > " + result);
		String arrowcolourdisplayed = Color.fromString(result).asHex();
		return arrowcolourdisplayed;
