...
//private By byAllLocations = By.cssSelector("div.template-root > div.hero.page-hero.content-module.hero-5 > div.featured-locations > a");
	...
	private By byAllLocations = By.xpath("/html/body/div[1]/div[1]/div[4]/a");
...
private By byVideoContainer = By.cssSelector("#connatix > div > a > cnx > cnx.cnx-highlights-container.cnx-is-desktop.cnx-size-md");
private By byVideoAutoPlay = By.cssSelector("#connatix > div > a > cnx > cnx.cnx-highlights-container.cnx-is-desktop.cnx-size-md > cnx.cnx-hl-padding-container > cnx > cnx.cnx-hl-slider-container > cnx:nth-child(1) > cnx > cnx > img");
//---private By byVideoAutoPlay = By.xpath("//div[contains(@class,'video-player')]/div[contains(@id,'jw-container-yYI3lbdK-L6hCQGol')]");
private By byVideoCMSContent = By.cssSelector("body > div.template-root > div.two-column-page-content > div.page-column-1 > div.content-module > div.content-module.zone-centerWell1 > a");
	private By byWeatherImage = By.cssSelector("div.two-column-page-content > div.page-column-1 > div > a > img");
	//private By byMoreMapsLink = By.xpath("/html/body/div[1]/div[4]/div/div[1]/div/a/div[2]/div");
	private By byMoreMapsLink = By.cssSelector("body > div.template-root > div:nth-child(4) > div > div.page-column-1 > div > a > div");
	 ...
	 */
	  ...
	  WebPageLoaded.isDomInteractive(); WebElement video =
	  getDriver().findElement(byVideoAutoPlay); video.scrollIntoView();
	  video.syncVisible(15); video.hover(); int xcoordinate =
	  video.getCoordinates().onPage().x;
	  System.out.println("xcoordinate xcoordinate... "+xcoordinate); int
	  ycoordinate = video.getCoordinates().onPage().y;
	  System.out.println("ycoordinate ycoordinate... "+ycoordinate);
	  getDriver().actions().moveToElement(video,
	  ...
	  getDriver().executeJavaScript("arguments[0].scrollIntoView(true);",
	  classattribute); 
	  ...
	  /*
	   * Method to verify Video is displayed on AccuWeather Landing page
	  */
	
	  public boolean isVideoDisplayed() throws InterruptedException {
	  WebPageLoaded.isDomInteractive(); 
	  //WebElement video = getDriver().findElement(byVideoAutoPlay); 
	  //video.scrollIntoView();
	  //video.syncVisible(15); 
	  ...
	  WebElement video = getDriver().findElement(byVideoContainer);
	  video.scrollIntoView();
	  video.syncVisible(15); 
	  video.hover(); 
	  int xcoordinate = video.getCoordinates().onPage().x;
	  ...
	  //getDriver().actions().moveToElement(video,0,1000).build().perform();
	  getDriver().executeJavaScript("arguments[0].scrollIntoView(true);",video);
	  getDriver().manage().timeouts().setScriptTimeout(250, TimeUnit.SECONDS);
	  WebElement videos = getDriver().findElement(byVideoAutoPlay);
	  getDriver().executeJavaScript("arguments[0].scrollIntoView(true);",videos);
	...
	/*
	 * Method to verify CMS Content is displayed in Landing Page
	*/
	public boolean isCMSContentDisplayed() {
		WebPageLoaded.isDomInteractive();
		WebElement cmscontent = getDriver().findElement(byVideoCMSContent);
		cmscontent.syncVisible(15);
		return cmscontent.isDisplayed();
	}
	 ...
	 */
	public void clickMoreMapsLink() throws InterruptedException {
		//WebPageLoaded.isDomInteractive();
		WebElement moremapslink = getDriver().findElement(byMoreMapsLink);
		WebPageLoaded.isDomInteractive(10000);
		getDriver().findElement(byMoreMapsLink);
		
		getDriver().executeJavaScript("arguments[0].scrollIntoView(true);" + "window.scrollBy(0,-100);",moremapslink);
		Thread.sleep(2000);
		getDriver().actions().moveToElement(moremapslink).build().perform();
