...
//private By byVideoAutoPlay = By.xpath("//div[contains(@class,'video-player')]/div[contains(@id,'jw-container-yYI3lbdK-L6hCQGol')]");
	...
	/**
	 * @author HFARAZ
	 *         Method to select from the list of cities that comes when user type
	 *         any city name in search bar on home page
	 */
	...
	public ArrayList readWeatherNewsTitles(int count)
	{
		...
		for (int i = 1; i <= count; i++)
		{
			if (i == 1)
			{
				int j = i + 1;
				if (getDriver()
						.findElement(By.cssSelector("div.zone-centerWell2 > div > a:nth-child(" + j
								+ ") > div.tout-body > div.tout-meta > span.tout-type"))
						.getText().equalsIgnoreCase("VIDEO"))
				{
					getDriver().findElement(By.cssSelector(
							"div.zone-centerWell2 > div > a:nth-child(" + j + ") > div.tout-body > div.tout-title > p"))
							.jsClick();
					/*
					...
					while (iterator.hasNext())
					{
						...
						if (!parentWindow.equals(childWindow))
						{
							...
							weatherNewsTitles.add(getDriver().findElement(By.cssSelector(
									"body > div > div.featured-video.featured-video > div.video-content > div.media-data > h1"))
									.getText());
							getDriver().close();
						}
					}
						...
						.getText().equalsIgnoreCase("VIDEO"))
				{
					...
					getDriver()
							.findElement(By.cssSelector(
									"div.zone-centerWell2 > div > a:nth-child(" + j + ") > div > div.tout-title > p"))
							.jsClick();
					/*
					 * window = getDriver().getWindowHandle(); switchToDifferentTab(window);
					 */
