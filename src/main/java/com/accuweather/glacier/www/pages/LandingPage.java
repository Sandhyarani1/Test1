...
public class LandingPage extends BasePage
{
	...
	// private By bySecondLocation = By.cssSelector("div.template-root >
	// div.hero.page-hero.content-module.hero-5 > div.featured-locations >
	// a:nth-child(2)");

	private By bySecondLocation = By.cssSelector("div.featured-locations > a:nth-child(2)");

	// private By bySecondLocation =
	// By.xpath("//body/div[1]/div[1]/div[4]/a[contains(@class,'recent-location-item
	// featured-location')]");
	...
	// private By byVideoAutoPlay =
	// By.xpath("//div[contains(@class,'video-player')]/div[contains(@id,'jw-container-yYI3lbdK-L6hCQGol')
	// and contains(@class,'jwplayer jw-reset jw-state')]");
	...
	private By byMoreMapsLink = By
			.cssSelector("div.two-column-page-content > div.page-column-1 > div > a > div.cta-text > div");
	...
	private By centerWell1Title = By
			.cssSelector("div.content-module.zone-centerWell1 > a > div > div > div.featured-title");
	private By centerWell1FeaturedDate = By
			.cssSelector("div.content-module.zone-centerWell1 > a > div > div > div.featured-date");
	private By centerWell1Image = By.cssSelector("div.content-module.zone-centerWell1 > a > div > img");
	private By centerWell1VideoTitle = By
			.cssSelector("div.title-share > div.jw-title.jw-reset-text > div.jw-title-primary.jw-reset-text");
	private By weatherNewsArticles = By.cssSelector("div.zone-centerWell2 > div > a");
	private By rightRail1_Articles = By.cssSelector("div.content-module.zone-rightRail1 > div > a");
	private By rightRail2_Articles = By.cssSelector("div.zone-rightRail2 > div > a");
	...
	public String getVideoClassAttribute() throws InterruptedException
	{
		...
		// getDriver().executeJavaScript("window.scrollBy(0,document.body.scrollHeight
	...
	/**
	 * 
	 * @author HFARAZ Method to check if the title of content module is displayed
	 * 
	...
	/**
	 * 
	 * @author HFARAZ Method to check if the image of content module is displayed
	 * 
	 * @return true if displayed else false
	 * 
	 */
	...
	/**
	 * 
	 * @author hasanfaraz Method to get the src of the image of content module
	 * 
	 */
	...
	public int getCountOfWeatherNewsArticles()
	{
		...
		try
		{
			...
			{
				noOfWeatherNewsArticles = weatherArticles.size() - 1;
			}
			else
				noOfWeatherNewsArticles = 0;
		}
		...
		return noOfWeatherNewsArticles;

	}

	/**
	 * 
	 * @author HFARAZ Method to click 'SHOW MORE' button for Weather News
	 * 
	 */
	...
	/**
	 * 
	 * @author HFARAZ Method to read the titles of weather news articles
	 * 
	...
	public ArrayList readWeatherNewsTitles(int count)
	{
		...
		for (int i = 1; i <= count; i++)
		{
			...
			{
				int j = i + 1;
				if (getDriver().findElement(By.cssSelector("div.zone-centerWell2 > div > a:nth-child(" + j
						+ ") > div.tout-body > div.tout-meta > span.tout-type")).getText().equalsIgnoreCase("VIDEO"))
				{
					getDriver().findElement(By.cssSelector(
							"div.zone-centerWell2 > div > a:nth-child(" + j + ") > div.tout-body > div.tout-title > p")).jsClick();
					...
					while (iterator.hasNext())
					{
						...
						if (!parentWindow.equals(childWindow))
						{
							...
							//Adding the news titles
							weatherNewsTitles.add(getDriver().findElement(By.cssSelector(
									"body > div > div.featured-video.featured-video > div.video-content > div.media-data > h1")).getText());
							
							//Adding the URLS to the array list
							weatherNewsArticles_URLs.add(getDriver().getCurrentUrl());
			...
			else
			{
						...
						+ ") > div.tout-body > div.tout-meta > span.tout-type")).getText().equalsIgnoreCase("VIDEO"))
				{
					String parentWindow = getDriver().getWindowHandle();
					getDriver().findElement(By.cssSelector(
									"div.zone-centerWell2 > div > a:nth-child(" + j + ") > div > div.tout-title > p")).jsClick();
					...
					while (iterator.hasNext())
					{
						...
						if (!parentWindow.equals(childWindow))
						{
							...
							//Adding the news titles
							weatherNewsTitles.add(getDriver().findElement(By.cssSelector("body > div > div.featured-video.featured-video > div.video-content > div.media-data > h1"))
									.getText());
				...
				else
				{
					...
					//Adding the news titles
					weatherNewsTitles.add(getDriver().findElement(By.cssSelector(
							"body > div.template-root > div:nth-child(5) > div > div.page-column-1 > div:nth-child(1) > div.article.card.full-mobile-width.comments > h1.title.hasPartner"))
							.getText());
	...
	{
		int noOfRightRail1_Articles = 0;
		WebPageLoaded.isDomComplete();
		scrollDown(800);
		WebPageLoaded.isDomComplete();
		try
		{
			List<WebElement> rightRailArticles = getDriver().findElements(rightRail1_Articles);
			if (!rightRailArticles.isEmpty())
			{
				noOfRightRail1_Articles = rightRailArticles.size();
			}
			else
				noOfRightRail1_Articles = 0;
		}
		catch (NoSuchElementException e)
		{
			noOfRightRail1_Articles = 0;
		...
		return noOfRightRail1_Articles;
	...
	public int getCountOfRightRail2_Articles()
	{
		...
		try
		{
			List<WebElement> rightRailArticles = getDriver().findElements(rightRail2_Articles);
			if (!rightRailArticles.isEmpty())
			{
				noOfRightRail2_Articles = rightRailArticles.size();
			}
			else
				noOfRightRail2_Articles = 0;
		}
		catch (NoSuchElementException e)
		{
			noOfRightRail2_Articles = 0;
		...
		return noOfRightRail2_Articles;
